...
angular.module('flowableModeler')
    .run(['$rootScope', 'formioComponents', '$timeout',
        function($rootScope, formioComponents, $timeout) {
            ...
            $rootScope.$on('formUpdate', function(event, form) {
                angular.merge($rootScope.contentFormsBuilder, form);
            ...
            var originalComps = _.cloneDeep($rootScope.contentFormsBuilder.components);
            $rootScope.jsonCollapsed = true;
            $timeout(function() {
                $rootScope.jsonCollapsed = false;
            }, 200);
            var currentDisplay = 'form';
            $rootScope.$watch('contentFormsBuilder.display', function(display) {
                if (display && (display !== currentDisplay)) {
                    currentDisplay = display;
                    if (display === 'form') {
                        $rootScope.contentFormsBuilder.components = originalComps;
                    } else {
                        $rootScope.contentFormsBuilder.components = [{
                            type: 'panel',
                            input: false,
                            title: 'Page 1',
                            theme: 'default',
    ...
    .controller('FormsBuilderEditorController', ['$rootScope', '$scope', '$translate', '$http', '$timeout', '$location', '$modal', '$routeParams', '$popover', 'FormBuilderService',
