  ...
  .controller('FormsBuilderCtrl', ['$rootScope', '$scope', '$translate', '$http', '$timeout','$location', '$modal', function ($rootScope, $scope, $translate, $http, $timeout, $location, $modal) {
	  ...
	  $scope.createForm = function() {
		  ...
		  $scope.createFormCallback = function(result) {
			  ...
			  $location.path("/forms-builder-editor/" + result.id);
		  };
	  ...
	  $scope.editFormDetails = function(form) {
		  if (form) {
		      $rootScope.editorHistory = [];
			  $location.path("/forms-builder-editor/" + form.id);
		  }
	  };
        ...
        $http({method: 'POST', url: FLOWABLE.APP_URL.getModelsUrl(), data: $scope.model.form})
			.then(function(data, status, headers, config) {
                ...
                if ($scope.createFormCallback) {
                	$scope.createFormCallback(data.data);
				...
				$http({method: 'POST', url: FLOWABLE.APP_URL.getCloneModelsUrl($scope.model.form.id), data: $scope.model.form})
					.then(function(data, status, headers, config) {
						...
						if ($scope.duplicateFormCallback) {
							$scope.duplicateFormCallback(data.data);
